...
public class ResourceChanger : Feature {
    private static bool _isAfterR129;
    public static Color PlanetColor;
    public static Color TitleColor;
    public static Color TileColor;
    public static string ResourcePackName;
    private static Sprite _autoSprite;
    private static ResourceChangerSetting _settings;
    ...
    public ResourceChanger() : base(Main.Instance, nameof(ResourceChanger), true, typeof(ResourceChanger), typeof(ResourceChangerSetting)) {
        _isAfterR129 = typeof(scrPlanet).Field("planetarySystem") != null;
        Patch();
        ...
        _settings = (ResourceChangerSetting) Setting;
    }

    private void Patch() {
        Type type = typeof(scrPlanet);
        if(_isAfterR129) type = type.Assembly.GetType("PlanetRenderer");
    ...
    protected override void OnEnable() {
        if(ADOBase.isLevelSelect && _settings.ChangeTileColor) VersionSafe.LoadScene(ADOBase.sceneName);
        else if(ADOBase.controller) {
            if(_settings.ChangeRabbit) LoadRabbit();
            if(_settings.ChangeBallColor) LoadPlanet();
        }
    }
    ...
    protected override async void OnDisable() {
        try {
            ...
            if(ADOBase.isLevelSelect && _settings.ChangeTileColor) VersionSafe.LoadScene(ADOBase.sceneName);
                ...
                if(_settings.ChangeRabbit) UnloadRabbit();
                if(_settings.ChangeBallColor) UnloadPlanet();
                if(_settings.ChangeTileColor) UnloadTileColor();
            }
        } catch (Exception e) {
            Main.Instance.LogReportException("Failed to disable resource changer", e);
        }
    }
    ...
    protected override void OnGUI() {
        ...
        settingGUI.AddSettingToggle(ref _settings.ChangeRabbit, localization["resourceChanger.changeRabbit"], () => {
            if(_settings.ChangeRabbit) LoadRabbit();
            else UnloadRabbit();
        });
        settingGUI.AddSettingToggle(ref _settings.ChangeBallColor, localization["resourceChanger.changeBallColor"], () => {
            if(_settings.ChangeBallColor) LoadPlanet();
            else UnloadPlanet();
        });
        settingGUI.AddSettingToggle(ref _settings.ChangeTileColor, localization["resourceChanger.changeTileColor"], () => {
            if(ADOBase.isLevelSelect) VersionSafe.LoadScene(ADOBase.sceneName);
            else if(!_settings.ChangeTileColor) UnloadTileColor();
        });
    }
    ...
    private static List<scrPlanet> GetAllPlanets() {
        ...
        if(_isAfterR129) obj = obj.GetValue("planetarySystem");
    ...
    private static void UnloadRabbit() {
        if(!_autoSprite || !ADOBase.editor) return;
        ADOBase.editor.autoImage.sprite = _autoSprite;
        ADOBase.editor.Invoke("OttoUpdate");
    }

    private static void UnloadPlanet() {
        if(_isAfterR129) foreach(scrPlanet planet in GetAllPlanets()) planet.Invoke("LoadPlanetColor");
    ...
    private class ResourceChangerSetting(JAMod mod, JObject jsonObject = null) : JASetting(mod, jsonObject) {
    ...
    public static void OnFloorStart(scrFloor __instance) {
        if(!_settings.ChangeTileColor || __instance.tag != "Beat") return;
    ...
    public static void OnPlanetStart(scrPlanet __instance) {
        if(!_settings.ChangeBallColor || VersionSafe.IsCoopMode()) return;
        object obj = __instance;
        if(_isAfterR129) obj = obj.GetValue("planetRenderer");
    ...
    public static bool OnPlanetAndLogoColorChange() => !_settings.ChangeBallColor || VersionSafe.IsCoopMode();

    public static void Prefix(ref Color color) {
        if(_settings.ChangeBallColor && !VersionSafe.IsCoopMode()) color = PlanetColor;
    }

    [JAPatch(typeof(scrFloor), nameof(scrFloor.SetTileColor), PatchType.Prefix, false, TryingCatch = false)]
    public static bool OnTileColorChange(scrFloor __instance) => !_settings.ChangeTileColor || __instance.tag != "Beat";
    ...
    public static void OnLogoTextAwake(scrLogoText __instance) {
        ...
        if(_settings.ChangeBallColor && !VersionSafe.IsCoopMode()) {
