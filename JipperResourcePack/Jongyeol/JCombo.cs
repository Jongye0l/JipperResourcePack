...
public class JCombo : Combo {
    private static JComboSettings _settings;
    protected override void AddPatch() {
        Patcher.AddPatch(OnHit2);
    }

    public JCombo() : base(typeof(JComboSettings)) {
        _settings = (JComboSettings) Setting;
    }

    protected override void OnGUI() {
        base.OnGUI();
        Main.SettingGUI.AddSettingToggle(ref _settings.YellowCombo, Main.Instance.Localization["combo.yellowCombo"]);
    }
    ...
    private static void OnHit2(HitMargin hit) {
        ...
        switch(hit) {
            ...
            case HitMargin.Auto when _settings.EnableAutoCombo:
    ...
    private class JComboSettings(JAMod mod, JObject jsonObject = null) : ComboSettings(mod, jsonObject) {
